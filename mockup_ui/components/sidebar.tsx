        ...
        >
          ...
          <div className="flex-1 py-6 px-2 space-y-1 overflow-y-auto">
            {routes.map((route) => {
              ...
              const isActive =
                pathname === route.path ||
                (route.path === "/dashboard/settings" && pathname.startsWith("/dashboard/settings"))
                ...
                <Link key={route.path} href={route.path}>
                  <Button
                    ...
                    className={cn(
                      ...
                      isActive && "bg-[rgba(255,255,255,0.1)] text-[#01cdfe] neon-border",
                      expanded ? "px-4" : "px-0 justify-center",
                    )}
                  >
                    <Icon className={cn("h-5 w-5", isActive && "text-[#01cdfe]")} />
                    <span
                      className={cn(
      ...
      >
        <div className="p-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <span className="text-xl font-bold text-white neon-text ml-2">Wolf</span>
          </div>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="py-6 px-4 space-y-1">
          {routes.map((route) => {
            ...
            return (
              <Link key={route.path} href={route.path} onClick={() => setMobileOpen(false)}>
                <Button
                  variant="ghost"
                  className={cn(
                    "w-full justify-start text-gray-400 hover:text-white hover:bg-[rgba(255,255,255,0.05)]",
                ...
                >
                  <Icon className={cn("mr-2 h-5 w-5", isActive && "text-[#01cdfe]")} />
                  {route.name}
                </Button>
              </Link>
            )
